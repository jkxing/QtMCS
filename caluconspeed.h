#ifndef CALUCONSPEED_H
#define CALUCONSPEED_H
#include <iostream>
#include <vector>
#include <fstream>
#include <sstream>
...
public:
    caluconspeed();
    double inputspeed1,inputspeed2;
    struct edge
{
    double v; //流速
    double leng; //管道长度
...
void initrect(){
    ...
    vector<double> tmp(EDGESUM+1,0);
    tmp[EDGESUM-5] = 1;
    tmp[EDGESUM] = inputspeed1;
    addrect(tmp);
    vector<double> temp(EDGESUM+1,0);
    temp[EDGESUM-4] = 1;
    temp[EDGESUM] = inputspeed2;
    addrect(temp);
...
vector<double> calc(int num, vector<double>&length, int i1, int i2, int o1, int o2, int o3,double v1,double v2)
{
    ...
    edges[EDGESUM-4].v = inputspeed1;
    edges[EDGESUM-5].v = inputspeed2;
